...
public class Algoritmo2 extends AbstractAlgoritmo implements
	 ...
	 * @param temporada
	 * @param pronosticos
	 *            Lista de pron�sticos prerrellena en la que s�lo falta rellenar
	...
	private void predecir(Temporada temporada,
		...
		if (!partidosYaJugados.isEmpty()) {
			...
			// Predicci�n de salidas
			Map<String, RealVector> resultados = testNeuralNetwork(
					myMlPerceptron,
					rellenarMatricesPrediccion(pronosticos, numeroParametros));

			// Para los resultados obtenidos, se obtendr� una
			// predicci�n
			System.out.println("Pron�stico: " + resultados.toString());
	...
	private void generarPronosticos(final Map<String, RealVector> resultados,
		...
		// Habr� 'x' empates en primera, e 'y' en segunda. Ser�n los m�s
		// probables
		// en esa jornada. Para el resto, ser� 1 � 2, seg�n su probabilidad de
		...
		// Pron�sticos ordenados por probabilidad de empates
		...
		for (PronosticoPartido pronostico : pronosticos) {
			...
			for (PronosticoPartido pronosticoEmpate : pronosticosConEmpates) {
				if (pronosticoEmpate.getPartido().getID()
					...
					// No se fija empate. Se toma 1 o 2. El m�s probable.
					// Mejora: Si est�n pr�ximas esas probabilidades, se toma el
					// equipo con mejor clasificaci�n en la liga.
					if (pronostico.getPorcentaje1() >= pronostico
						...
						if (pronostico.getPorcentaje1() > pronostico
							...
							ParametroEquipo posicionVisitante = pronostico
									...
									.getParametro(
											ParametroNombre.POSICION_EN_CLASIFICACION);
							// Si est� m�s abajo en la tabla de clasificaci�n,
							// pierde.
			...
			// TODO: OJO: ����LOS EQUIPOS Y PR�N�STICO PUEDE QUE SE PINTEN
			// DADOS
	...
	private MultiLayerPerceptron crearRedNeuronal(
		...
		// Fijado seg�n indicaciones de:
		// http://www.heatonresearch.com/node/707
		// Aumento hasta 2f seg�n:
		...
		System.out.println("N�mero de neuronas de capa oculta: "
	...
	private Map<String, RealVector> rellenarMatricesPrediccion(
		...
		System.out.println("Rellenando matrices de predicci�n...");
		...
		System.out.println("Matrices de predicci�n llenas.");
	...
	private void normalizarMatriz(RealMatrix matrixInputs) throws Exception {
		...
		// 1�. Normalizaci�n por columnas. Se normaliza linealmente entre 0
		// y 1
		...
		for (int col = 0; col < matrixInputs.getColumnDimension(); col++) {
			...
			if (diferencia == 0) {
				// Se ha comentado, dado que el n�mero de la jornada es un
				// par�metro, y siempre es el mismo para los partidos a predecir
				// throw new Exception(
	...
	public void handleLearningEvent(LearningEvent event) {
		...
		if (bp.getCurrentIteration() % 5000 == 0)
			System.out.println("Iteraci�n: " + bp.getCurrentIteration()
